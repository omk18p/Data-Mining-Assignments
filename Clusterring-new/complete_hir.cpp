...
double completeLinkageDistance(const vector<int> &c1, const vector<int> &c2, const vector<Point> &data) {
    double maxDist = 0.0;
    for (int i : c1) {
        for (int j : c2) {
            maxDist = max(maxDist, euclideanDistance(data[i], data[j]));
        }
    }
    return maxDist;
}
...
int main() {
    ...
    while (clusters.size() > 1) {
        ...
        // Find pair with smallest complete linkage distance
        for (size_t i = 0; i < clusters.size(); i++) {
            for (size_t j = i + 1; j < clusters.size(); j++) {
                double d = completeLinkageDistance(clusters[i], clusters[j], data);
                if (d < minDist) {
                    minDist = d;
                    mergeA = i;
                    mergeB = j;
    ...
    // Output to CSV
    ofstream fout("hierarchical_complete_out.csv");
    fout << "X,Y,cluster\n";
